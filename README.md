...
Automated Reasoning for Ryu — decide whether an answer FOLLOWS from a written policy using a decision procedure written from scratch (exact rational arithmetic, finite-domain enumeration, Fourier–Motzkin, branch-and-bound), so a verdict carries the minimal rule set that decided it and a concrete counterexample.
...
- Binary: `ryu-reasoning` from the [Ryu releases](https://github.com/amajorai/ryu/releases).
- Crate: `cargo install ryu-reasoning`.
...
Decide whether an answer **follows from** a written policy, using a solver instead of
a second opinion.

Ask a model "is this answer right?" and you get another answer that can be wrong in
the same way. This app asks a different question: given the policy, *is there any
world in which this answer is false?* If there is, it shows you one. If there isn't,
that is a proof, and it names the rules that produced it.

```
  policy document ──(model)──▶ variables + rules ──(you edit)──▶ policy
                                                                    │
...
A model is used at the two **language** edges, because turning prose into logic is a
language problem. It is used **nowhere in the middle**. The verdict, the
counterexamples, the rules a finding blames, and the suggested corrections all come
out of a decision procedure.
...
belong in it, and it is the single highest-leverage field in a policy.
...
declared in the manifest's own `mcp_servers`. `reasoning__solve` is the id a workflow
`mcp` node takes, and it is deterministic and offline, so a branch gated on a proof
always takes the same path for the same inputs. `reasoning__policies` lists a policy's
...
where `reasoning__check` exists at all; without a runner, delegation falls back to a
