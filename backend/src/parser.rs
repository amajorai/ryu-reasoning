...
impl<'a> Parser<'a> {
    ...
    fn parse_product(&mut self) -> Result<Node, ParseError> {
        ...
        loop {
            ...
            let combined = if divide {
                if !b.is_constant() {
                    return Err(self.err(
                        "division by a variable is not supported — rules must stay linear",
                    ));
                }
                a.scale(
                    Rat::ONE
    ...
    fn parse_unary(&mut self) -> Result<Node, ParseError> {
        if self.eat(&Tok::Minus) {
            ...
            return Ok(Node::Term(
                term.neg().ok_or_else(|| self.err("arithmetic overflowed"))?,
            ));
        }
    ...
    fn as_formula(&self, node: Node) -> Result<Expr, ParseError> {
        match node {
            ...
            Node::Str(s) => Err(self.err(format!(
                "the text \"{s}\" is not a condition on its own"
            ))),
    ...
    fn as_term(&self, node: Node) -> Result<Linear, ParseError> {
        match node {
            ...
            Node::EnumRef(v) => Err(self.err(format!(
                "'{v}' is an enum and cannot be used in arithmetic"
            ))),
        }
    }
}
