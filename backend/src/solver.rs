...
fn check_theory(env: &Env, assignment: &HashMap<String, bool>, ctx: &mut Ctx) -> TheoryResult {
        ...
        match fourier_motzkin(&work, ctx) {
            ...
            LinearResult::Feasible(model) => {
                ...
                let (Some(minus_floor), Some(plus_ceil)) = (
                    floor.checked_neg().and_then(|v| Rat::new(v, 1)),
                    Rat::new(ceil, 1),
                ) else {
                    ctx.exhausted = Some("integer bound overflowed".into());
                    return LinearResult::Unknown;
...
mod tests {
    ...
    fn simple_conjunction_is_satisfiable() {
        let out = decide(
            &env(),
            &[p("is_manager"), p("tenure_months >= 12")],
            &Budget::default(),
        );
    ...
    fn disjunction_explores_both_arms() {
        let out = decide(
            &env(),
            &[
                p("tenure_months <= 5 or tenure_months >= 100"),
                p("tenure_months >= 50"),
            ],
            &Budget::default(),
        );
    ...
    fn a_starved_budget_returns_unknown_not_a_verdict() {
        ...
        let out = decide(
            &env(),
            &[
                p("is_manager or tenure_months >= 12"),
                p("not is_manager"),
                p("tenure_months <= 5"),
            ],
            &budget,
        ...
        assert!(
            out.is_unknown(),
            "a starved search must not claim unsat: {out:?}"
        );
    }
