...
//! tools appear as `reasoning.solve`, `reasoning.check`, … — which is exactly the
...
async fn call_tool(ctx: &Arc<Ctx>, name: &str, args: Value) -> Result<Value> {
    ...
    let load = |id: &str| -> Result<crate::policy::Policy> {
        ctx.store.get(id)?.ok_or_else(|| {
            anyhow::anyhow!("no policy with the id '{id}' — call `policies` to list them")
        })
    };
